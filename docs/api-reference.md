...
| `closeDialog()`                 | `Promise<void>`                   | Requests dismissal, cancels pending opening, or rejects while submitting.    |
...
Submission rejects both a pending close decision and any new close request with
`EditorOperationBusyError`, without cancelling persistence or invoking
`beforeClose`. Cancellation during opening returns to `ready` without open or
close events. Successful form submissions close by default; set
`editing.dialog.closeOnSuccess: false` to retain the canonical form.
